...
object JavaScriptParser {
  ...
  private val nodeJsFixWrapper: Seq[String] => Seq[String] = {
    ...
    case l if l.length == 2 =>
      Seq(s"(function (exports, require, module, __filename, __dirname) { ${l.head}", s"${l(1)} });")
    case l =>
  ...
  private class ErrorNodeCollector(private val errorNodes: mutable.Buffer[ErrorNode] = mutable.ListBuffer.empty)
